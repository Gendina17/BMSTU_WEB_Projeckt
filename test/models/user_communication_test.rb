...
class UserCommunicationTest < ActiveSupport::TestCase
  ...
  test 'test_get_db' do
    ...
    user1.save
    user2 = User.new(id: 222_222, email: 'test2@mail.ru', password: '123456', first_name: 'Олегин', last_name: 'Милох' )
    user2.save
    instance = UserCommunication.new(liker: user1.id, like: user2.id )
    assert instance.save
    assert_equal 333_333, UserCommunication.find_by_like(222_222).liker
    id = instance.id
    assert_equal 222_222, UserCommunication.find_by_id(id).like
  end
end
